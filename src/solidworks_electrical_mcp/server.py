...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "SCHEMATIC DRAWING is NOT scriptable: placing library symbols fails — "
        "newEwProjectSymbol / newEwProjectSymbolFromSymbolType / setEwSymbolName "
        "all return 2 (EW_BAD_INPUTS) on insert. A folio's existing symbols can be "
        "READ (getProjectSymbolsFromFileID) and MOVED (setX/YPosition), and wires "
        "drawn (newEwProjectLine(0) -> setStart/EndPointX/YPosition -> insert, rc 0), "
        "and macros placed (insertMacroAt(name,x,y)), but device symbols cannot be "
        "created from the library. So the electrical DATA model is fully scriptable; "
        "drawing the schematic graphically is not.\n\n"
